...
class ListImgDataset(Dataset):
    ...
    def load_img_from_file(self, f_path):
        ...
        targets = self.load_label(label_path, cur_img.shape[:2]) if os.path.exists(
            label_path) else None
    ...
    def load_label(self, label_path: str, img_size: tuple):
...
class Detector(object):
    def __init__(self, args, checkpoint_id=None, model=None, seq_num=2):
        ...
        img_list = os.listdir(os.path.join(self.args.rmot_path, 'BDD/images/track/train', self.seq_num[0]))
        img_list = [os.path.join(self.args.rmot_path, 'BDD/images/track/train', self.seq_num[0], _)
                    for _ in img_list if ('jpg' in _) or ('png' in _)]

        self.img_list = sorted(img_list)
        self.img_len = len(self.img_list)

        self.json_path = os.path.join(self.args.rmot_path, 'BDD/expression', seq_num[0], seq_num[1])
    ...
    def detect(self, prob_threshold=0.6, ref_threshold=0.2, area_threshold=100):
        ...
        for i, data in enumerate(tqdm(loader)):
            ...
            res = self.detr.inference_single_image(cur_img.cuda().float(), sentences=self.sentence,
            ...
            active_indx = []
            full_indx = torch.arange(len(dt_instances), device=dt_instances.scores.device)
            for id in torch.unique(dt_instances.obj_idxes):
                indx = torch.where(dt_instances.obj_idxes == id)[0]
                active_indx.append(full_indx[indx][dt_instances.scores[indx].argmax()])
            if len(active_indx):
                active_indx = torch.stack(active_indx)
                dt_instances = dt_instances[active_indx]
        ...
        self.write_gt(gt_path, self.json_path,
                      os.path.join(args.rmot_path, 'BDD/labels_with_ids', self.seq_num[0]), seq_h, seq_w)
...
if __name__ == '__main__':
    ...
    detr = load_model(args, detr, args.resume)
    detr.eval()
    detr = detr.cuda()

    expressions_root = os.path.join(args.rmot_path, 'BDD/expression')
    video_ids = [
        '0000f77c-cb820c98',
        '00abd8a7-ecd6fc56',
        '00fcfec9-7ba5dabf',
    ...
    for seq_num in seq_nums:
        ...
        det.detect(prob_threshold=args.prob_threshold, ref_threshold=args.refer_threshold)
