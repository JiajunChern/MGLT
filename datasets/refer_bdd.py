...
import copy
import json
import datasets.transforms_refer_kitti as T
...
class DetMOTDetection:
    def __init__(self, args, data_txt_path: str, seqs_folder, dataset2transform):
        ...
        self.label_files = [
            (x.replace('images/track/train', 'labels_with_ids').replace('.png', '.txt').replace('.jpg', '.txt'))
    ...
    def _pre_single_frame(self, idx: int, expression_infos):
        ...
        for expression_info in expression_infos:
            ...
            if expression_info is not None:
                frame_id = int(img_path.split('/')[-1].split('.')[0].split('-')[-1])
        ...
        for obj_idx, label in enumerate(labels):
            ...
            for i in range(len(ref_ids_list)):
                if int(label[1]) in ref_ids_list[i]:  # if an id is in ref_ids, then mask it as 1, else mask 0
                    targets['is_ref'][obj_idx].append(1.0)
                else:
                    targets['is_ref'][obj_idx].append(0.0)
    ...
    def _get_sample_range(self, start_idx):
        ...
        assert self.sample_mode in ['fixed_interval', 'random_interval'], 'invalid sample mode: {}'.format(
            self.sample_mode)
    ...
    def _pre_batch_sentence(self, idx: int, labels, sentences):
        img_path = self.img_files[idx]
        if 'BDD' in img_path:
            video_id = img_path.split('/')[-2]
            expression_list = os.listdir(osp.join(self.args.rmot_path, 'BDD/expression', video_id))
            expression_random = random.choice(expression_list)
            expression_path = osp.join(self.args.rmot_path, 'BDD/expression', video_id, expression_random)
        ...
        # TODO: limit
        key = str(expression_info['label'])
        # TODO: make sure that one different sample
        if key in labels and labels[key] >= self.args.num_sentences - 1:
            return None
        if expression_info['sentence'] in sentences:
            return None
    ...
    def __getitem__(self, idx):
        ...
        if 'BDD' in img_path:
            ...
            with open(expression_path, 'r') as f:
                expression_info = json.load(f)
            sentences.append(expression_info['sentence'])
            labels[str(expression_info['label'])] = 0
            expression_infos.append(expression_info)
        else:
            raise NotImplementedError()

        # TODO: select sentence in a mini-batch
        while len(sentences) < self.args.num_sentences:
            b_expression_info = self._pre_batch_sentence(idx, labels, set(sentences))
            if b_expression_info is not None:
                key = str(expression_info['label'])
                if key in labels:
                    labels[key] = labels[key] + 1
                else:
                    labels[key] = 0
                sentences.append(b_expression_info['sentence'])
                expression_infos.append(b_expression_info)

        images, targets = self.pre_continuous_frames(sample_start, sample_end, sample_interval, expression_infos)
        data = {}
        ...
        data_dict['gt_instances'] = gt_instances
...
def build_dataset2transform(args, image_set):
    ...
    dataset2transform_train = {'BDD': kitti_train}
    if image_set == 'train':
...
def build(image_set, args):
    ...
    assert root.exists(), f'provided Ref-BDD path {root} does not exist'
