...
class DetMOTDetection:
    def __init__(self, args, data_txt_path: str, seqs_folder, dataset2transform):
        ...
        self.label_files = [(x.replace('images', 'labels_with_ids').replace('training', 'labels_with_ids').replace(
    ...
    def _pre_batch_sentence(self, idx: int, labels, sentences, num_sentences):
        img_path = self.img_files[idx]
        if 'KITTI' in img_path:
        ...
        if key in labels and labels[key] >= num_sentences - 1:
            return None
    ...
    def __getitem__(self, idx):
        ...
        if self.args.random_sentences:
            num_sentences = random.randint(1, self.args.num_sentences)
        else:
            num_sentences = self.args.num_sentences
        while len(sentences) < num_sentences:
            b_expression_info = self._pre_batch_sentence(idx, labels, set(sentences), num_sentences)
