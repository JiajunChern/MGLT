...
def load_model(args, model, model_path, optimizer=None, resume=False,
               lr=None, lr_step=None):
    start_epoch = 0
    checkpoint = torch.load(model_path, map_location=lambda storage, loc: storage)
    print(f'loaded {model_path}')
    ...
    # detr_name = 'r50_deformable_detr_plus_iterative_bbox_refinement-checkpoint.pth'
    # load_clip = model_path.split('/')[1] in detr_name and args.backbone == 'weights/RN50.pt'
    # for k in list(state_dict.keys()):
    #     # TODO: Drop the backbone parameters in deformable DETR when loading CLIP
    #     if 'backbone' in k and load_clip:
    #         print('Drop parameter {}.'.format(k) + msg)
    #         state_dict.pop(k)
