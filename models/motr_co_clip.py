...
from .fusion_helper import FeatureResizer, VL_Align, PMA
from .matcher import build_matcher
from .memory_bank import build_memory_bank
from .deformable_transformer_plus_bi_attn import build_deforamble_transformer, pos2posemb
    ...
    def loss_refers(self, outputs, gt_instances: List[Instances], indices: List[tuple], num_boxes):
        ...
        if self.focal_loss:
            loss_ce = sigmoid_focal_loss(
                ...
                alpha=0.25,
                gamma=2,
                num_boxes=num_boxes, mean_in_dim1=False) / src_refs.shape[-1]  # * self.clip_loss_decay
            loss_ce = loss_ce.sum()
        else:
            loss_ce = F.cross_entropy(src_refs, gt_labels_target)
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        if not (track_instances.obj_idxes != -1).any():  # 没有跟踪
            outputs_i = {
                ...
                'pred_refers': pred_refers_i.transpose(0, 1),
            }
            ...
            for i, ind in enumerate(indices):
                ...
                active_idxes = (track_instances.obj_idxes[:, i] >= 0) & (
                        track_instances.matched_gt_idxes[:, i] >= 0)  # 当前帧能够匹配到的目标
                ...
                if len(active_track_boxes) > 0:
                    ...
                    track_instances.iou[active_idxes, i] = matched_boxlist_iou(Boxes(active_track_boxes),
                                                                               Boxes(gt_boxes))
            ...
            if 'aux_outputs' in outputs:  # 此处匹配时对新生儿时重新算对应关系的，不直接使用最后一层box输出的对应关系
                for i, aux_outputs in enumerate(outputs['aux_outputs']):
                    ...
                    matched_indices_layer = [(ind[0].to(pred_logits_i.device), ind[1].to(pred_logits_i.device)) for ind
                                             in matched_indices_layer]
            ...
            def match_for_single_decoder_layer(unmatched_outputs, matcher, untracked_gt_instances,
                ...
                new_track_indices = matcher(unmatched_outputs,
                                            ...
                                            g_size=self.g_size)  # list[tuple(src_idx, tgt_idx)]
            ...
            for ibn, gt_ins in enumerate(gt_instances_i):
                ...
                prev_matched_indices = torch.stack(
                    [full_track_idxes[matched_track_idxes], track_instances.matched_gt_idxes[matched_track_idxes, ibn]],
                    dim=1)  # 检测或跟踪与gt的对应关系
                ...
                unmatched_outputs = {
                    ...
                    'pred_refers': track_instances.pred_refers[unmatched_track_idxes, ibn].unsqueeze(0)

                }
                # new_matched_indices = match_for_single_decoder_layer(unmatched_outputs, self.matcher,
                # untracked_gt_instances, unmatched_track_idxes, untracked_tgt_indexes)
                new_matched_indices = match_for_single_decoder_layer(unmatched_outputs, self.matcher,
                                                                     untracked_gt_instances, unmatched_track_idxes,
                                                                     untracked_tgt_indexes)

                # step5. update obj_idxes according to the new matching result. 分配GT的ID给track和GT所在的索引
                track_instances.obj_idxes[new_matched_indices[:, 0], ibn] = gt_ins.obj_ids[
                    new_matched_indices[:, 1]].long()
                track_instances.matched_gt_idxes[new_matched_indices[:, 0], ibn] = new_matched_indices[:, 1]

                # step6. calculate iou.
                active_idxes = (track_instances.obj_idxes[:, ibn] >= 0) & (
                        track_instances.matched_gt_idxes[:, ibn] >= 0)  # 当前帧能够匹配到的目标
                ...
                if len(active_track_boxes) > 0:
                    ...
                    active_track_boxes = box_ops.box_cxcywh_to_xyxy(active_track_boxes)
                    gt_boxes = box_ops.box_cxcywh_to_xyxy(gt_boxes)
                    track_instances.iou[active_idxes, ibn] = matched_boxlist_iou(Boxes(active_track_boxes),
                                                                                 Boxes(gt_boxes))
                ...
                outputs_i = {
                    ...
                    'pred_refers': pred_refers_i[:, ibn].unsqueeze(0),
                }
                for loss in self.losses:
                ...
                if 'aux_outputs' in outputs:  # 此处匹配时对新生儿时重新算对应关系的，不直接使用最后一层box输出的对应关系
                    for i, aux_outputs in enumerate(outputs['aux_outputs']):
                        ...
                        new_matched_indices_layer = match_for_single_decoder_layer(
                            unmatched_outputs_layer,
                            self.matcher, gt_ins[full_tgt_idxes],
                        ...
                        outputs_layer = {
                            ...
                            'pred_refers': aux_outputs['pred_refers'][ibn].unsqueeze(0),
                        }
                        for loss in self.losses:
                            if loss == 'masks':
                                # Intermediate masks losses are too costly to compute, we ignore them.
...
class RuntimeTrackerBase(object):  # 实际为一个跟踪ID分配器
    ...
    def update(self, track_instances: Instances, g_size=1):
        ...
        track_instances.obj_idxes[new_obj, 0] = (self.max_obj_id + torch.arange(num_new_objs, device=device)).view(-1,
                                                                                                                   1).repeat(
            1, g_size).view(-1)  # 分配ID
        self.max_obj_id += num_new_objs  # max_obj_id为已有多人ID

        track_instances.disappear_time[disappeared_obj] += 1
        to_del = disappeared_obj & (
                track_instances.disappear_time[:, 0] >= self.miss_tolerance)  # 假如当前帧检测不到，且消失很长时间，则把ID删掉，
...
class TrackerPostProcess(nn.Module):
    ...
    def forward(self, track_instances: Instances, target_size) -> Instances:
        ...
        if len(prob):
            ...
            scores, labels = prob.reshape(-1, self.g_size, bn, cls_num).max(1)[0].reshape(
                -1, 1, bn, cls_num
            ).repeat(1, self.g_size, 1, 1).reshape(-1, bn, cls_num).max(-1)
        else:
            scores = out_logits[..., 0].sigmoid()
            labels = torch.full_like(scores, 0, dtype=torch.long)

        # refers = out_refers.sigmoid().max(-1)[0]
        if out_refers.shape[0] != 0:
            refers, _ = out_refers.sigmoid().max(-1)
        else:
            refers = scores
...
class MOTR(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
                 ...
                 query_denoise=0, g_size=1, args=None):
        ...
        """
        super().__init__()
        # TODO: Adapter to RMOT task
        self.refer_embed = VL_Align(txt_dim=512)
        ...
        if freeze_text_encoder:
            for p in self.track_embed.text_encoder.parameters():
        ...
        self.text_pooler = PMA(1024, 8, 1)
        self.num_sentences = 1
        init_values = 1e-4
        self.gamma = nn.Parameter(init_values * torch.ones((1024)), requires_grad=True)
    ...
    def _generate_empty_tracks(self, g_size=1, batch_size=1):
        ...
        track_instances.ref_pts = self.position.weight.view(-1, 1, 4).repeat(1, g_size, 1).view(
            -1, 4) + self.position_offset.weight
        track_instances.query_pos = self.query_embed.weight.view(-1, 1, d_model).repeat(1, g_size, 1).view(
            -1, d_model) + self.query_embed_offset.weight
        ...
        track_instances.output_embedding = torch.zeros((len(track_instances), batch_size, d_model),
                                                       device=device)  # motr decode输出的feature，把这个输入qim中可以获得track的query，某个目标跟踪过程中不再使用query_pos
        track_instances.obj_idxes = torch.full((len(track_instances), batch_size), -1, dtype=torch.long,
                                               device=device)  # ID
        track_instances.matched_gt_idxes = torch.full((len(track_instances), batch_size), -1, dtype=torch.long,
                                                      device=device)  # 与匹配到的gt在该图片中的索引
        track_instances.disappear_time = torch.zeros((len(track_instances), batch_size), dtype=torch.long,
                                                     device=device)  # 消失时间，假如目标跟踪多久后删除该目标
        track_instances.iou = torch.zeros((len(track_instances), batch_size), dtype=torch.float,
                                          device=device)  # 与对应GT的IOU
        track_instances.scores = torch.zeros((len(track_instances), batch_size), dtype=torch.float,
                                             device=device)  # 实际是当前帧检测输出的置信度
        track_instances.track_scores = torch.zeros((len(track_instances), batch_size), dtype=torch.float, device=device)
        track_instances.pred_boxes = torch.zeros((len(track_instances), batch_size, 4), dtype=torch.float,
                                                 device=device)  # 检测或跟踪query输出的box框
        track_instances.pred_logits = torch.zeros((len(track_instances), batch_size, self.num_classes),
                                                  dtype=torch.float, device=device)  # 检测或跟踪query输出的置信度argsigmod
        track_instances.pred_refers = torch.zeros((len(track_instances), batch_size, 1),
                                                  dtype=torch.float, device=device)
        if self.training:
            track_instances.pred_refers = torch.zeros((len(track_instances), batch_size, self.num_sentences),
                                                      dtype=torch.float, device=device)

        track_instances.group_ids = torch.arange(g_size, dtype=torch.long, device=device).repeat(num_queries).view(-1,
                                                                                                                   1).repeat(
            1, batch_size)
        ...
        track_instances.mem_bank = torch.zeros((len(track_instances), batch_size, mem_bank_len, d_model),
                                               dtype=torch.float32, device=device)  # 存储历史的output_embedding（经过了一个linear）
        track_instances.mem_padding_mask = torch.ones((len(track_instances), batch_size, mem_bank_len),
                                                      dtype=torch.bool, device=device)  # 是否更新了，假如更新为0，不更新为1
        track_instances.save_period = torch.zeros((len(track_instances), batch_size), dtype=torch.float32,
                                                  device=device)  #
    ...
    def _forward_single_image(self, samples, track_instances: Instances, sentences, gtboxes=None):
        features, pos = self.backbone(samples)  # 通过backbone获取多次度feature+pad mask+position
        src, mask = features[-1].decompose()
        assert mask is not None

        self.num_sentences = len(sentences)
        tokenized_texts = self.track_embed.tokenize(sentences, src.device)
        text_sentence_embeddings, text_word_embeddings = self.track_embed.text_encoder(tokenized_texts, None,
                                                                                       'onlytextword')
        text_word_poolers = self.text_pooler(text_word_embeddings).reshape(1, self.num_sentences, 1024)
        text_sentence_embeddings = text_sentence_embeddings + self.gamma * text_word_poolers
        text_sentence_features = self.txt_proj(text_sentence_embeddings)
        ...
        text_word_masks = torch.ones(size=text_sentence_features.shape[:2], device=text_sentence_features.device,
                                     requires_grad=False) == 1
        language_dict_features = {"hidden": text_sentence_features, "masks": text_word_masks}

        # transformer 包括encode + decode
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, language_dict_features, memory = \
            self.transformer(srcs, masks, pos, query_embed, ref_pts=ref_pts,
                             ...
                             attn_mask=attn_mask, language_dict_features=language_dict_features)
        ...
        for lvl in range(hs.shape[0]):  # 检测头包括box（xc,yc,w,h并归一化1）和class分支
            ...
            outputs_refer = self.refer_embed[lvl](hs[lvl], language_dict_features['hidden'])
        ...
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1],
               'pred_refers': outputs_refer[-1]}  # 最后一次迭代的输出
        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord, outputs_refer)  # 中间过程迭代的输出
        out['hs'] = hs[-1]
        out['context'] = memory
        return out
    ...
    def _post_process_single_image(self, frame_res, track_instances, is_last):
        if self.query_denoise > 0:  # 由query内有部分噪声GT，因此需要单独拿出来，并放在了ps_outputs下
            ...
            ps_refers = frame_res['pred_refers'][:, n_ins:]
        ...
        track_instances.pred_refers = frame_res['pred_refers'].transpose(0, 1)
        ...
        if not is_last:  # 经过这步后将仅保留有ID的目标，且更新了track的query和pos
            ...
            # 更新跟踪的query，用于下一帧（检测query为学习的，跟踪query则为上一帧输出经过qim变换的特征）
    ...
    # 获取当前帧的跟踪框
    @torch.no_grad()
    def inference_single_image(self, img, sentences, ori_img_size, track_instances=None):
        if not isinstance(img, NestedTensor):
            img = nested_tensor_from_tensor_list(img)  # 补pad并或者pad的mask
        if track_instances is None:
            # 初始化decode的输入或者说目标query，包括（query+pose）
            track_instances = self._generate_empty_tracks(g_size=self.g_size)
        else:
            track_instances = Instances.cat([
                self._generate_empty_tracks(g_size=self.g_size),
                track_instances])
        ...
        res = self._forward_single_image(
            ...
            sentences=sentences
        )  # backbone+encode+decode，获得decode的输出和中间迭代过程输出的box和最后输出的feat(hs,作为经过QIM后可作为下一帧的query)
    ...
    def forward(self, data: dict):
        ...
        self.num_sentences = len(data['sentences'])
        outputs = {
            'pred_logits': [],
            'pred_boxes': [],
            'pred_refers': [],
        ...
        for frame_index, (frame, gt) in enumerate(zip(frames, data['gt_instances'])):
            ...
            if track_instances is None:
                ...
                track_instances = Instances.cat([
                    self._generate_empty_tracks(g_size=self.g_size, batch_size=nbatch), track_instances])
            # track_instances = self._generate_empty_tracks(proposals)
            if self.use_checkpoint and frame_index < len(frames) - 1:
                def fn(frame, gtboxes, *args):
                    frame = nested_tensor_from_tensor_list(frame)
                    tmp = Instances((1, 1), **dict(zip(keys, args)))
                    frame_res = self._forward_single_image(frame, tmp, sentences, gtboxes)
                    return (
                ...
                frame_res = {
                    ...
                    'aux_outputs': [{
                        ...
                        'pred_refers': tmp[4 + 5 + 5 + i],
                    } for i in range(5)],
                    'context': tmp[-1],
                }
            else:
                frame = nested_tensor_from_tensor_list(frame)
                frame_res = self._forward_single_image(frame, track_instances, sentences, gtboxes)
        ...
        for i in range(num_frames_per_batch):
            ...
            for j in range(args.dec_layers):
                weight_dict.update({"frame_{}_ps{}_loss_ce".format(i, j): args.cls_loss_coef,
                                    ...
                                    'frame_{}_ps{}_loss_refer'.format(i, j): args.refer_loss_coef,
                                    })
...
def img():
    ...
    image = np.ascontiguousarray(((img.tensors[0].permute(1, 2, 0).cpu() * torch.tensor(
        [0.229, 0.224, 0.225]) + torch.tensor([0.485, 0.456, 0.406])) * 255).numpy().astype(np.uint8))
    ...
    for i in range(68):
        ...
        for box in bboxes[5 * i:5 * (i + 1)]:
            cv2.rectangle(image_copy, pt1=(int(box[0, 0]), int(box[0, 1])), pt2=(int(box[1, 0]), int(box[1, 1])),
