            ...
            for ibn, gt_ins in enumerate(gt_instances_i):
                ...
                if 'aux_outputs' in outputs:  # 此处匹配时对新生儿时重新算对应关系的，不直接使用最后一层box输出的对应关系
                    for i, aux_outputs in enumerate(outputs['aux_outputs']):
                        ...
                        new_matched_indices_layer = match_for_single_decoder_layer(unmatched_outputs_layer,
...
class TrackerPostProcess(nn.Module):
    ...
    def forward(self, track_instances: Instances, target_size) -> Instances:
        ...
        if len(prob):
            ...
            scores, labels = prob.reshape(-1, self.g_size, bn, cls_num).max(1)[0].reshape(-1, 1, bn, cls_num).repeat(1,
                                                                                                                     self.g_size,
                                                                                                                     1,
                                                                                                                     1).reshape(
                -1, bn, cls_num).max(-1)
...
class MOTR(nn.Module):
    ...
    def _generate_empty_tracks(self, proposals=None, g_size=1, batch_size=1):
        ...
        track_instances.ref_pts = self.position.weight.view(-1, 1, 4).repeat(1, g_size, 1).view(-1,
                                                                                                4) + self.position_offset.weight
        track_instances.query_pos = self.query_embed.weight.view(-1, 1, d_model).repeat(1, g_size, 1).view(-1,
                                                                                                           d_model) + self.query_embed_offset.weight
    ...
    def _post_process_single_image(self, frame_res, track_instances, is_last):
        ...
        if not is_last:  # 经过这步后将仅保留有ID的目标，且更新了track的query和pos
            out_track_instances = self.track_embed(tmp,
                                                   g_size=self.g_size)  # 更新跟踪的query，用于下一帧（检测query为学习的，跟踪query则为上一帧输出经过qim变换的特征）
    ...
    def inference_single_image(self, img, ori_img_size, track_instances=None, proposals=None):
        ...
        if track_instances is None:
            track_instances = self._generate_empty_tracks(proposals,
                                                          g_size=self.g_size)  # 初始化decode的输入或者说目标query，包括（query+pose）
        else:
