...
class CLIPResNetWithAttention(nn.Module):
    ...
    def __init__(self, layers, output_dim=1024, input_resolution=224, width=64, pretrained=None, att_level3=False,
                    ...
                    if 'positional_embedding' in new_k:
                        if self.attnpool.positional_embedding.shape != state_dict[new_k].shape:
                            ...
                            spatial_pos = F.interpolate(
                                state_dict[new_k][1:, ].reshape(1, 7, 7, cls_pos.shape[1]).permute(0, 3, 1, 2),
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, drop_path_rate=0.):
        ...
        self.resblocks = nn.Sequential(
            *[ResidualAttentionBlock(width, heads, self.attn_mask, dpr[i]) for i in range(layers)])
...
class CLIPTextContextEncoder(nn.Module):
    ...
    def forward(self, text, context, mode):
        if mode == 'onlytext':
            ...
            w = x[torch.arange(x.shape[0]), ::] @ self.text_projection
            # w = w.reshape(1, K, self.embed_dim)
            x = x[torch.arange(x.shape[0]), eos_indx] @ self.text_projection
            x = x.reshape(1, K, self.embed_dim)  # extend
            return x, w
            ...
            # query + class
            x_text = self.token_embedding(text)  # n_class, n_text, C
            _, N1, C = x_text.shape  # 1 14 512
            B, N2, K, C = context.shape  # 1 1 4 512

            eos_indx = (text.argmax(dim=-1) + N2).expand(K)
            eos_indx = eos_indx.reshape(1, K).expand(B, K).reshape(-1)  # 1 4

            x_text = x_text.reshape(1, 1, N1, C).expand(B, K, N1, C)  # 1 4 14 512
            context = context.permute(0, 2, 1, 3)  # 1 4 1 512

            x = torch.cat([x_text[:, :, 0:1], context, x_text[:, :, 1:]], dim=2).reshape(B * K, N1 + N2, C)

            # 48 track description pool
            pool_text = self.token_embedding(track_text)
            K3, N3, C3 = pool_text.shape  # 4 15 512

            eos_indx3 = track_text.argmax(dim=-1)
            eos_indx3 = eos_indx3.reshape(1, K3).expand(1, K3).reshape(-1)

            x = torch.cat([x, pool_text], dim=0)  # 59 15 512
            x = x + self.positional_embedding
