...
from .fusion_helper import VL_Fuse
    ...
    def in_projection(self, q: Tensor, k: Tensor, v: Tensor) -> List[Tensor]:
        ...
        if k is v:
            ...
            # encoder-decoder attention
                ...
                b_q, b_kv = None if self.in_proj.bias is None else self.in_proj.bias.split(
            ...
            b_q, b_k, b_v = None if self.in_proj.bias is None else self.in_proj.bias.split(
                [self.embed_dim, self.k_dim, self.v_dim])
            return F.linear(q, w_q, b_q), F.linear(k, w_k, b_k), F.linear(v, w_v, b_v)

    def attention(self, q: Tensor, k: Tensor, v: Tensor, attn_bias: Optional[Tensor] = None,
...
class MultiheadAttentionWithScore(MultiheadAttention):
    def __init__(self, embed_dim: int, num_heads: int,
        ...
        super().__init__(embed_dim, num_heads, attention_dropout, scale_factor, bias, add_bias_kv, add_zero_attn, k_dim,
                         v_dim, batch_first, **kwargs)
    ...
    def attention(self, q: Tensor, k: Tensor, v: Tensor, attn_bias: Optional[Tensor] = None,
        ...
        if attn_mask is not None:
            ...
            mask_weights.scatter_(2, attn.argmax(-1)[..., None],
                                  torch.gather(attn_mask, dim=2, index=attn.argmax(-1)[..., None]))
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        fusion_layer = VL_Fuse(txt_dim=512)
        self.encoder = DeformableTransformerEncoder(encoder_layer, num_encoder_layers, fusion_layer, 1)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, ref_pts=None, mem_bank=None, mem_bank_pad_mask=None,
                attn_mask=None, language_dict_features=None):
        ...
        memory = self.encoder(src_flatten, spatial_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten,
                              mask_flatten, language_dict_features)
        memory, language_dict_features = memory["visual"], memory["language"]
        ...
        if self.two_stage:
            ...
            reference_points = torch.cat([reference_points_two, ref_pts.unsqueeze(0).expand(bs, -1, -1)[:, topk:]],
                                         axis=1)
        ...
        if self.two_stage:
            return hs, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord_unact, language_dict_features, memory
        return hs, init_reference_out, inter_references_out, None, None, language_dict_features, memory
...
class DeformableTransformerDecoupleHead(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                          ...
                                                          sigmoid_attn=sigmoid_attn, extra_track_attn=extra_track_attn,
                                                          memory_bank=memory_bank, im2col_step=im2col_step)
        self.decoder = DeformableTransformerDecoderDecoupleHead(decoder_layer, num_decoder_layers,
...
class DeformableTransformerAttnWithScore(DeformableTransformerAttn):
    def __init__(self, d_model=256, nhead=8,
        ...
        super().__init__(d_model, nhead, num_encoder_layers, num_decoder_layers, dim_feedforward, dropout, activation,
                         return_intermediate_dec, num_feature_levels, dec_n_points, enc_n_points, two_stage,
...
class DeformableTransformerEncoder(nn.Module):
    def __init__(self, encoder_layer, num_layers, fusion_layer, num_fusion_layer):
        super().__init__()
        self.fusion_layers = _get_clones_advanced(fusion_layer, num_layers, num_fusion_layer)
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None,
        ...
        # output = src
        output = {"visual": src, "language": language_dict_features}
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device)
        for _, (layer, fusion_layer) in enumerate(zip(self.layers, self.fusion_layers)):
            # TODO: To employ bi-direction cross attention
            if not isinstance(fusion_layer, nn.Identity):
                output = fusion_layer(output)
            output["visual"] = layer(
                output["visual"], pos, reference_points, spatial_shapes, level_start_index, padding_mask
            )
        return output
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def _forward_self_cross_output_weight(self, tgt, query_pos, reference_points, src, src_spatial_shapes,
                                          level_start_index,
    ...
    def forward_output_weight(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index,
                              src_padding_mask=None, mem_bank=None, mem_bank_pad_mask=None, attn_mask=None, lid=None):
        if self.self_cross:
...
class DeformableTransformerDecoderLayerWithScore(DeformableTransformerDecoderLayer):
    def __init__(self, d_model=256, d_ffn=1024,
                 ...
                 n_levels=4, n_heads=8, n_points=4, self_cross=True, sigmoid_attn=False,
                 extra_track_attn=False, memory_bank=False, im2col_step=64):
        super().__init__(d_model, d_ffn, dropout, activation, n_levels, n_heads, n_points, self_cross, sigmoid_attn,
                         extra_track_attn, memory_bank, im2col_step)
...
class DeformableTransformerDecoderDecoupleHead(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:  # 迭代优化，获取下一次迭代的参考点
                tmp = torch.cat(
                    [self.bbox_embed[lid](output[:, :query_num]), self.bbox_embed_trk[lid](output[:, query_num:])],
                    axis=1)
                if reference_points.shape[-1] == 4:
                    new_reference_points = tmp + inverse_sigmoid(reference_points)
                    new_reference_points = new_reference_points.sigmoid()
...
class DeformableTransformerDecoderMixer(nn.Module):
    ...
    def forward(self, tgt, tgt_two, reference_points, reference_points_two, src, src_spatial_shapes,
        ...
        for lid, (layer, layer_mixer) in enumerate(zip(self.layers, self.layers_mixer)):
            ...
            if self.return_intermediate:
                ...
                intermediate_reference_points_two.append(
                    reference_points_two.transpose(0, 1).reshape(bs, num_queries // self.g_size, 1, -1).repeat(1, 1,
                                                                                                               self.g_size,
                                                                                                               1).reshape(
                        bs, num_queries, -1))
                intermediate_class_two.append(
                    outputs_class_two.transpose(0, 1).reshape(bs, num_queries // self.g_size, 1, -1).repeat(1, 1,
                                                                                                            self.g_size,
                                                                                                            1).reshape(
                        bs, num_queries, -1))
        ...
        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(intermediate_reference_points), torch.stack(
                intermediate_class), torch.stack(intermediate_reference_points_two), torch.stack(intermediate_class_two)
...
class DeformableTransformerMixer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.decoder = DeformableTransformerDecoderMixer(decoder_layer, decoder_layer_mixer, num_decoder_layers,
                                                         return_intermediate_dec, g_size=g_size)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, query_embed_two=None, ref_pts=None, ref_pts_two=None,
        ...
        if self.two_stage:
            ...
            init_reference_out = reference_points
        # decoder， 输入 query+query_pos(即reference_points)+encode的输出，输出N*256的特征
        hs, intermediate_reference_points, intermediate_class, intermediate_reference_points_two, intermediate_class_two = self.decoder(
            tgt, tgt_two, reference_points, reference_points_two, memory,
            spatial_shapes, level_start_index,
            valid_ratios, mask_flatten,
            mem_bank, mem_bank_pad_mask, attn_mask)
